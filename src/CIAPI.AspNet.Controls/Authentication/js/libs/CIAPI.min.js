...
(function(a,b,c){CIAPI.store=function(d){if(d.storageType.match(/^localStorage/i)){return a.store.localStorage(d.key,d.value,d)}else{if(d.storageType.match(/^sessionStorage/i)){return a.store.sessionStorage(d.key,d.value,d)}else{return a.store(d.key,d.value,d)}}}})(amplify,_);
(function(a,b,c){a.request.decoders.ciapiDecoder=function(g,e,i,h,f){var d=function(j){CIAPI.publish("CIAPI.request.error",j);f(j)};if(e==="timeout"){d(new CIAPI.dto.ApiErrorResponseDTO(503,503,"Request timed out when contacting server"));return}if(e==="error"){if(!b(g).isNull()&&!b(g).isUndefined(g)){d(g);return}d(new CIAPI.dto.ApiErrorResponseDTO(403,1,"The server is refusing to respond to this request.  Are you making an HTTPS request from an HTTP page?"));return}if(!b(g).isNull()&&!b(g).isUndefined(g)&&!g.ErrorCode){h(g)}else{d(g)}};a.request.define("createSession","cors",{url:"{ServiceUri}/session?only200=true",timeout:5000,dataType:"json",contentType:"application/json; charset=utf-8",type:"POST",processData:false,beforeSend:function(e,d){d.data=JSON.stringify(d.data);return true},decoder:"ciapiDecoder"});a.request.define("DeleteSession","cors",{url:"{ServiceUri}/session?_method=DELETE&only200=true",dataType:"json",contentType:"application/json; charset=utf-8",type:"POST",processData:false,beforeSend:function(e,d){d.data=JSON.stringify(d.data);return true},decoder:"ciapiDecoder"});a.request.define("ValidateSession","cors",{url:"{ServiceUri}/UserAccount/ClientAndTradingAccount?UserName={UserName}&Session={Session}&only200=true&ts={ts}",dataType:"json",contentType:"application/json; charset=utf-8",type:"GET",processData:false,beforeSend:function(e,d){d.data=JSON.stringify(d.data);return true},decoder:"ciapiDecoder"});a.request.define("ListCfdMarkets","cors",{url:"{ServiceUri}/cfd/markets?MarketName={searchByMarketName}&MarketCode={searchByMarketCode}&ClientAccountId={clientAccountId}&MaxResults={maxResults}&only200=true",dataType:"json",contentType:"application/json; charset=utf-8",type:"GET",decoder:"ciapiDecoder"});a.request.define("GetPriceBars","cors",{url:"{ServiceUri}/market/{marketId}/barhistory?interval={interval}&span={span}&priceBars={maxResults}&only200=true",dataType:"json",contentType:"application/json; charset=utf-8",type:"GET",decoder:"ciapiDecoder"})})(amplify,_);
var CIAPI=CIAPI||{};(function(a,b,g){var e=8*60*60*1000,f="localStorage",d=function(h){CIAPI.store({key:"CIAPI_connection",value:h,storageType:f,expires:e})},c=function(){CIAPI.connection.isConnected=false;CIAPI.connection.UserName="";CIAPI.connection.Session="";CIAPI.connection.ServiceUri="";CIAPI.connection.StreamUri="";d(CIAPI.connection)};CIAPI.connection={isConnected:false,UserName:"",Session:"",ServiceUri:"",StreamUri:""};CIAPI.connect=function(h){b(h).defaults({UserName:"not_specified",Password:"not_specified",ServiceUri:"not_specified",StreamUri:"not_specified",success:function(){},error:function(){}});a.request({resourceId:"createSession",data:{ServiceUri:h.ServiceUri,UserName:h.UserName,Password:h.Password},success:function(i){CIAPI.connection.isConnected=true;CIAPI.connection.Session=i.Session;CIAPI.connection.UserName=h.UserName;CIAPI.connection.ServiceUri=h.ServiceUri;CIAPI.connection.StreamUri=h.StreamUri;d(CIAPI.connection);CIAPI.publish("CIAPI.connection.status",CIAPI.connection);h.success(i)},error:function(i){b(i).defaults({ErrorCode:0,ErrorMessage:"Unknown",HttpStatus:0});c();CIAPI.publish("CIAPI.connection.status",CIAPI.connection);h.error(i)}})};CIAPI.reconnect=function(){CIAPI.connection=b({}).extend(CIAPI.store({key:"CIAPI_connection",storageType:f}));if(!CIAPI.connection.isConnected){CIAPI.publish("CIAPI.connection.status",CIAPI.connection);return}a.request({resourceId:"ValidateSession",data:{ServiceUri:CIAPI.connection.ServiceUri,UserName:CIAPI.connection.UserName,Session:CIAPI.connection.Session,ts:(new Date()).getTime()},success:function(h){CIAPI.publish("CIAPI.connection.status",CIAPI.connection)}})};CIAPI.subscribe("CIAPI.request.error",function(h){if(h===null){return}if(h.HttpStatus===401){c();CIAPI.publish("CIAPI.connection.status",CIAPI.connection)}});CIAPI.disconnect=function(h){h=h||{};b(h).defaults({UserName:CIAPI.connection.UserName,Session:CIAPI.connection.Session,ServiceUri:CIAPI.connection.ServiceUri,success:function(){},error:function(){}});a.request({resourceId:"DeleteSession",data:{ServiceUri:h.ServiceUri,UserName:h.UserName,Session:h.Session},success:function(i){c();CIAPI.publish("CIAPI.connection.status",CIAPI.connection);h.success(i)},error:function(i){h.error(i)}})};CIAPI.replaceConnectionTokens=function(h){if(!CIAPI.connection.isConnected){console.log("Warning:  replaceTokens should not be called before authentication has happened, or not all tokens will be replaced")}return h.replace("{CIAPI.connection.UserName}",CIAPI.connection.UserName).replace("{CIAPI.connection.Session}",CIAPI.connection.Session)}})(amplify,_);
...
CIAPI.dto.AccountInformationResponseDTO=function(d,c,b,a){this.LogonUserName=d;this.ClientAccountId=c;this.ClientAccountCurrency=b;this.TradingAccounts=a;Object.freeze(this)};
...
CIAPI.dto.SessionDeletionResponseDTO=function(a){this.LoggedOut=a;Object.freeze(this)};
