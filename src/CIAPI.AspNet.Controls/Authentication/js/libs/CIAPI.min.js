...
var CIAPI=CIAPI||{};(function(b,j,c){var k=8*60*60*1000,d=20*60*1000;var e=function(l){CIAPI.store({key:"CIAPI_connection",value:l,storageType:"sessionStorage",expires:k});if(l.isConnected){g()}else{a()}};var i=function(){CIAPI.connection=j({}).extend(CIAPI.store({key:"CIAPI_connection",storageType:"sessionStorage"}));j(CIAPI.connection).defaults({isConnected:false,UserName:"",Session:"",ServiceUri:"",StreamUri:""})};var h=function(){CIAPI.connection.isConnected=false;CIAPI.connection.UserName="";CIAPI.connection.Session="";e(CIAPI.connection)};CIAPI.connect=function(m){j(m).defaults({UserName:CIAPI.connection.UserName,Password:c,ServiceUri:CIAPI.connection.ServiceUri,StreamUri:CIAPI.connection.StreamUri,success:function(){},error:function(){}});var l=b.request({resourceId:"createSession",data:{ServiceUri:m.ServiceUri,UserName:m.UserName,Password:m.Password},success:function(n){CIAPI.connection.isConnected=true;CIAPI.connection.UserName=m.UserName;CIAPI.connection.Session=n.Session;e(CIAPI.connection);m.success(n)},error:function(n){j(n).defaults({ErrorCode:0,ErrorMessage:"Unknown",HttpStatus:0});h();m.error(n)}})};CIAPI.disconnect=function(l){l=l||{};j(l).defaults({UserName:CIAPI.connection.UserName,Session:CIAPI.connection.Session,ServiceUri:CIAPI.connection.ServiceUri,success:function(){},error:function(){}});b.request({resourceId:"DeleteSession",data:{ServiceUri:l.ServiceUri,UserName:l.UserName,Session:l.Session},success:function(m){h();l.success(m)},error:function(m){l.error(m)}})};CIAPI.OnConnectionInvalid=function(l){};CIAPI.validateConnection=function(){if(!CIAPI.connection.isConnected){return}b.request({resourceId:"GetClientAndTradingAccount",data:{ServiceUri:CIAPI.connection.ServiceUri,UserName:CIAPI.connection.UserName,Session:CIAPI.connection.Session},success:function(l){},error:function(l){if(l.HttpStatus===401){h();CIAPI.OnConnectionInvalid(CIAPI.connection)}}})};var f=false;var g=function(){f=true;(function l(){if(f){setTimeout(function(){CIAPI.validateConnection();l()},d)}})()};var a=function(){f=false};i();CIAPI.validateConnection()})(amplify,_);
