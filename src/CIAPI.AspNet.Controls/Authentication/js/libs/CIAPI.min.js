...
(function(a,b,c){CIAPI.store=function(d){return a.store(d.key,d.value,d)}})(amplify,_);
...
var CIAPI=CIAPI||{};(function(a,b,f){var e=8*60*60*1000,d=function(g){CIAPI.store({key:"CIAPI_connection",value:g,expires:e})},c=function(){CIAPI.connection.isConnected=false;CIAPI.connection.UserName="";CIAPI.connection.Session="";CIAPI.connection.ServiceUri="";CIAPI.connection.StreamUri="";d(CIAPI.connection)};CIAPI.connection={isConnected:false,UserName:"",Session:"",ServiceUri:"",StreamUri:""};CIAPI.connect=function(g){b(g).defaults({UserName:"not_specified",Password:"not_specified",ServiceUri:"not_specified",StreamUri:"not_specified",success:function(){},error:function(){}});a.request({resourceId:"createSession",data:{ServiceUri:g.ServiceUri,UserName:g.UserName,Password:g.Password},success:function(h){CIAPI.connection.isConnected=true;CIAPI.connection.Session=h.Session;CIAPI.connection.UserName=g.UserName;CIAPI.connection.ServiceUri=g.ServiceUri;CIAPI.connection.StreamUri=g.StreamUri;d(CIAPI.connection);CIAPI.publish("CIAPI.connection.status",CIAPI.connection);g.success(h)},error:function(h){b(h).defaults({ErrorCode:0,ErrorMessage:"Unknown",HttpStatus:0});c();CIAPI.publish("CIAPI.connection.status",CIAPI.connection);g.error(h)}})};CIAPI.reconnect=function(){CIAPI.connection=b({}).extend(CIAPI.store({key:"CIAPI_connection"}));if(!CIAPI.connection.isConnected){CIAPI.publish("CIAPI.connection.status",CIAPI.connection);return}a.request({resourceId:"ValidateSession",data:{ServiceUri:CIAPI.connection.ServiceUri,UserName:CIAPI.connection.UserName,Session:CIAPI.connection.Session,ts:(new Date()).getTime()},success:function(g){CIAPI.publish("CIAPI.connection.status",CIAPI.connection)}})};CIAPI.subscribe("CIAPI.request.error",function(g){if(g===null){return}if(g.HttpStatus===401){c();CIAPI.publish("CIAPI.connection.status",CIAPI.connection)}});CIAPI.disconnect=function(g){g=g||{};b(g).defaults({UserName:CIAPI.connection.UserName,Session:CIAPI.connection.Session,ServiceUri:CIAPI.connection.ServiceUri,success:function(){},error:function(){}});a.request({resourceId:"DeleteSession",data:{ServiceUri:g.ServiceUri,UserName:g.UserName,Session:g.Session},success:function(h){c();CIAPI.publish("CIAPI.connection.status",CIAPI.connection);g.success(h)},error:function(h){g.error(h)}})};CIAPI.replaceConnectionTokens=function(g){if(!CIAPI.connection.isConnected){console.log("Warning:  replaceTokens should not be called before authentication has happened, or not all tokens will be replaced")}return g.replace("{CIAPI.connection.UserName}",CIAPI.connection.UserName).replace("{CIAPI.connection.Session}",CIAPI.connection.Session)}})(amplify,_);
